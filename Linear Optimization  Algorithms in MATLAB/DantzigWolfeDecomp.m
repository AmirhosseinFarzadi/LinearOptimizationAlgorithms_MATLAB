...
% MATLAB code for Dantzig-Wolfe
% DantzigWolfeDecomp solves a linear programming with a special structure:
%                        min  c'*x
%                      s.t.  A*x <= b
%                              x >= 0
...
f_sub=[];
for k=1:K
    %obtain initial extreme points from struct sub for subproblems these are zero vectors,
    %so initial objective function values will be zero, v(k) is initial extreme point of subproblem k
    v_sub{k}=sub.v{k};
...
f_B=[f_s; f_sub];      %initial f_B i.e. the objective coefficient of the restricted MP
B=eye(length(x_B));    %initial basis for master problem
iter_num=0;            %counter
options=optimset('LargeScale', 'on', 'Simplex', 'off');%choose largescale LP solver in linprog
while iter_num>=0
    ...
    %           s.t.A_k*x^k <= b^k
    %                   x^k >= 0
    % If SP_k is unbounded, then Go to STEP3, else let x^k=(v_i*)^k denote the optimal basic feasible solution
    % and computer (r^k)_* = (sig_k)^* - pie_k^2. If r_min={(r^k)_*}>=0, then the current basis B is optimal,
    % else Go to STEP3.
    for k=1:K
    ...
    if isempty(find(r_min < 0)) || abs(r_min) <= 1e-8    %if reduced cost >=0, then optimal
        disp('problem solved')
        fval=0;
        ...
        for k=1:K                                        %convert to optimal solution for original LP
            ...
            fval=fval+sub.c{k}'*x{k};                    %convert to optimal obj val for original LP
        end
        x=cell2mat(x);
        exit_flag=0;
        break
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % If all subproblems are bounded and r_min={(r^k)_*}<0, then let t be the index of k in SP_k
        % such that r_min=(r^t)_*. Let alpha_bar=[q_i*^t e_t]^T=[L_t*v_i*^t e_t]^T where v_i*^t is
        % the optimal extreme points of SP_t and Go to STEP4. Else there is a subproblem SP_s
        % that is unbounded and so an extreme direction d_j*^s will be generated such that
        % [(c^s)^T-(pie^1)^T*L_s]*d_j*^s < 0 and so let alpha_bar = [(q_bar)_j*^s 0]^T =
        % [L_s*d_j*^s 0]^T and GO to STEP4.
        if length(find(exitflag==1)) == K %if all subproblems are bounded and r_min<0
            ...
            alpha_bar=[q_t; zeros(length(x_B)-length(q_t),1)]; %generate alpha_bar
        end
    end
    ...
    if isempty(d_flag) %if d>=0, unbounded
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Computer the step length alpha by minimum ratio test. Let l* be the index
        % of the basic variables then attains the minimum ratio alpha. Go to STEP 6.
        alpha=min(x_B(d_flag)./abs(d(d_flag))); %minimun ratio test
        ...
        %% Step 7: Basis Update
        %%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Let B_l* be the column associated with the leaving variable x_l*.Update the
        % basis matrix B by removing B_l* and adding the column alpha_bar, and update B_set.
        B(:,leave(1))=alpha_bar;              %update the basis B
    end
    iter_num=iter_num+1;
    f_B=[f_s; f_sub];                         %update f_B for next iteration
    bound(:,iter_num)=[f_B'*x_B + sum(sig); f_B'*x_B];%new lower and upper bound
end %Go to STEP 1